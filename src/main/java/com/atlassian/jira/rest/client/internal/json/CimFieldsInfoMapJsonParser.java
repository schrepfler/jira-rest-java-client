...
public class CimFieldsInfoMapJsonParser implements JsonObjectParser<Map<String, CimFieldInfo>> {
	...
	private Iterable<Object> parseAllowedValues(@Nullable JSONArray allowedValues, FieldSchema fieldSchema) throws JSONException {
		...
		if (allowedValuesJsonParser != null) {
			...
			final boolean isReleasedMultiVersionHistoryGH = "version".equals(fieldSchema.getType())
					&& "com.pyxis.greenhopper.jira:greenhopper-releasedmultiversionhistory".equals(fieldSchema.getCustom());

			if ((isProjectCF || isVersionCF || isMultiVersionCF || isReleasedMultiVersionHistoryGH) && allowedValues.get(0) instanceof JSONArray) {
